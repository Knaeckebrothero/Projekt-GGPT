"""
This script  loads match data from the riot api into the database.
...
# Gets the matches from the riot apis and saves them in the database.
while not done:
    # Chunk-wise retrieves match ids from the riot apis.
    try:
        for match_id in api.get_match_ids(puuid=puuid, start=start, count=100, api_key=key):
            response = api.get_match(match_id, key)
            #response = MatchDto(json.load(apis.get_match(match_id, key)))
            matches.append(response)
            match_count = match_count + 1
        start = start + 100
    # Breaks the loop on apis error
